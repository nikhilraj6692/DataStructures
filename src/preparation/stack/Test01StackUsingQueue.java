...
//implement using two queues...add all elements to second queue before adding a new element. This makes adding costlier
//in other case, pop operation can be made costlier also by adding all elements to second queue before removing element
class Stack2<T>{
...
//recursive call is used to just reverse the queue so that the key which was inserted first in the queue becomes last to be polled from
class Stack1<T>{
