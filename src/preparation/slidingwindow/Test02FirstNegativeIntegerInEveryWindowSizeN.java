...
public class Test02FirstNegativeIntegerInEveryWindowSizeN {
    ...
    private static void printFirstNegativeIntegerUsingQueueWithSameStructureAsPrevProblem(int[] arr, int n, int k) {
        int i=0, j=0;
    ...
    /*
    the idea is to add first k index in the queue for which numbers are negative. Then iterate from first k+1th element and print
     element one by one by peeking from the queue if the queue is not empty and then adding to the queue if the current index is
      negative. Finally take out the first index from the queue which is outside of the current window
     */
    //int arr[] = {12, -1, -7, 8, -15, 30, 16, 28};
    // 2 4
