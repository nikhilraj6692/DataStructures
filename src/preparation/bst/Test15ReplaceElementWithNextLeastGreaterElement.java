...
public class Test15ReplaceElementWithNextLeastGreaterElement
{
    ...
    /*
    can be done by bst by constructing a bst and then find inorder successor of each element.
    However, in place inorder
    successor can also be found by iterating the array from the end and then replace it with its
    successor at the time of inserting into bst
     */
    ...
    private static Node<Integer> insertAndFindSuccessor(Node<Integer> root, AtomicInteger successor,
        int val)
    {
        if (root == null)
        {
