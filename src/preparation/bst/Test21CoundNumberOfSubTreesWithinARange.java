...
public class Test21CoundNumberOfSubTreesWithinARange
{
    ...
    private static int countSubTreesWithinARangeIterative(Node<Integer> root, int low, int high)
    {
        ...
        while (!q.isEmpty())
        {
            ...
            if (null == temp.left || (null != temp.left && temp.left.data >= low
                && temp.left.data <= high))
            {
                left = true;
            }
            ...
            if (null == temp.right || (null != temp.right && temp.right.data >= low
                && temp.right.data <= high))
            {
                right = true;
            }
    ...
    private static boolean countSubTreesWithinARange(Node<Integer> root, AtomicInteger count,
        int low, int high)
    {
        if (root == null)
        {
        ...
        if (countSubTreesWithinARange(root.left, count, low, high) && countSubTreesWithinARange(
            root.right, count, low, high)
