...
public class Test06CheckIfBinaryTreeIsABST
{
    ...
    private static boolean checkIfTreeIsABSTWithRange(Node<Integer> root, int minValue,
        int maxValue)
    {
        if (root == null)
        {
        ...
        return checkIfTreeIsABSTWithRange(root.left, minValue, root.data)
            && checkIfTreeIsABSTWithRange(root.right, root.data, maxValue);
    }

    /*
    Inorder of a BST is always sorted. Check if rootNode is smaller or equal to prevNode then
    certainly it is not in BST.
    Recursive inorder will work. Since, we have to return boolean so return true in case you hit
    leaf node. Can be done with list instead of prev
    node
     */
    private static boolean checkIfTreeIsABST(Node<Integer> root, Node<Integer> prevNode)
    {
        ...
        //can be done with list..in this case just pass list from the function checkIfTreeIsABST
        // (root, new ArrayList<>())
        //in main function
        /*list.add(root.data);
