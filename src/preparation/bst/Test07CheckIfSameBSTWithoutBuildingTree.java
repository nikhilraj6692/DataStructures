...
public class Test07CheckIfSameBSTWithoutBuildingTree
{
    ...
    /*
    it checks if the first element of both arrays is same or not. If same, then it compares all
    the elements of both arrays with its
    corresponding first element. If it is smaller, then put it in the left subarray else right
    subarray of corresponding arrays.
    finally check if generated left subarray length of both arrays is same or not. Similarly for
    right subarray. And finally
    ...
    private static boolean isSameBST(int[] x, int[] y, int length)
    {
        ...
        return isSameBST(leftSubArrayX, leftSubArrayY, lX) && isSameBST(rightSubArrayX,
            rightSubArrayY, rX);
    }
}
