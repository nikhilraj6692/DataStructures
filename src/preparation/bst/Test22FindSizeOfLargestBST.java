...
public class Test22FindSizeOfLargestBST {
    public static void main(String[] args) {
        ...
        //find inorder first, then find the largest subarray with increasing sequence and finally find the middle and the
        //length of the sub array. below method is too lengthy but if we have to only find the length, it would be O(n)
        List<Integer> list = new ArrayList<>();
        findInorder(root, list);
        List<Integer> preOrderedList = new ArrayList<>();
        findPreorder(root, preOrderedList);

        findLargestIncreasingSubsequenceAndPrintLengthAlongWithMiddleElement(list.stream().mapToInt(i->i).toArray(),
                preOrderedList.stream().mapToInt(i->i).toArray());

        System.out.print("\n--------------------------------\n");
        //this method calculates at each and every node whether the tree is in BST or not. If the root is in bst then return
        //size of the bst, but this method only works if whole subtree is a bst like in buildTree19() comments
    ...
    //30 15 20 40 5 8 2
    private static void findLargestIncreasingSubsequenceAndPrintLengthAlongWithMiddleElement(int[] arr, int[] preOrderArray) {
        ...
        //now find the preorder of the nodes, separate out all the elements from original orray which actually is present in
        //sub array
        int[] subArray = Arrays.stream(preOrderArray).boxed().filter(x->inOrderedSubarray.contains(x)).mapToInt(x->x).toArray();

        System.out.print("The length of largest bst is :: " + max + " and the bst is rooted at :: " + subArray[0]);
    }
