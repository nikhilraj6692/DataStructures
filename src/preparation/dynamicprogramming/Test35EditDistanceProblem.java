...
/*
in this problem, we have to find minimum distance between two strings such that three operations
may be possible,
i.e. insert a character in another string to make one string equal to another, replace characters
 in one string
...
in this case if one string is empty then obvious answer is that we have to do 3 insertions here.
The rule applies
to vice versa also.
...
here, the last character of both are same, since we have to find out minimum operation so we will
 just have to
...
if last character is not same, then we have to do three operations and see that what could be the
 minimum value of
the three operations

a) if we insert last character of one string in another, then two cases will form
    i) adc,abd -> adcd, abd (insertion of d in first string) -> it will now become case 2 as both
     characters at the
    end are same and since we just move to left in both strings in this case, so resulting
    strings would be
    adc,ab  ====> fn(s1,s2,x,y-1)
    ii) adc,abd -> adc, abcc (insertion of c in second string) -> it will now become case 2 as
...
public class Test35EditDistanceProblem
{
    ...
    private static int findSubSequence(char[] a, char[] b, int x, int y, int[][] dp)
    {
        ...
        {
            dp[x][y] =
                1 + min(findSubSequence(a, b, x - 1, y, dp), findSubSequence(a, b, x, y - 1, dp),
