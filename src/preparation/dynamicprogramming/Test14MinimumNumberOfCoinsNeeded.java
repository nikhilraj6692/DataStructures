...
public class Test14MinimumNumberOfCoinsNeeded {
    public static void main(String[] args) {
        ...
        System.out.print("\n------------------------------------\n");
        //Arrays.sort(coins);
        System.out.println("Minimum coins required is "+ minCoinsMyRecursiveMethod(coins, size, sum, 0, 0) );
    }
    ...
    static int minCoinsMyRecursiveMethod(int coins[], int n, int sum, int sumTillNow, int count)
    {
        ...
        for(int i =0;i<n ;i++){
            if((sumTillNow+coins[i] <= sum)) {
                min = Math.min(min, minCoinsMyRecursiveMethod(coins, n, sum, sumTillNow + coins[i], count + 1));
            }
        }
    ...
    private static int minCoinsRecursive(int[] coins, int size, int sum) {
        ...
        if(coins[size-1] <= sum){
            return Math.min(1+minCoinsRecursive(coins, size, sum-coins[size-1]) , minCoinsRecursive(coins,size-1,sum));
        }else{
        ...
        for(int i=0;i<size+1;i++){
            for(int j=0;j<sum+1;j++){
                if(j==0)
                    ...
                    dp[i][j] = Integer.MAX_VALUE-1; //minimum no. of coins needed to find a sum of any value except 0
