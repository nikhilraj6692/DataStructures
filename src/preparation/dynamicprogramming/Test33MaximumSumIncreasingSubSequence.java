...
/*
it is a variation of LIS. In LIS, we make use of previous length at jth position to update length
 at ith position. In this one,
...
public class Test33MaximumSumIncreasingSubSequence
{
    ...
    private static int MSIS(int[] arr)
    {
        ...
        for (int i = 1; i < arr.length; i++)
        {
            for (int j = 0; j < i; j++)
            {
                //doing same as printing sequence...as we have already created array for previous
                // elements so putting that
