...
public class Test22PrintShortesCommonSuperSequence {
    ...
    private static String printShortestSuperSeq(String x, String y) {
        ...
        /*
        print all shortest super sequence with lcs dp...remember always print all sequences with lcs dp
         */
    ...
    private static void printLPS(int[][] dp, String x, String y, int m, int n, Stack<Character> stack) {
        int i = m;
        int j = n;
    ...
    public static void printAllLongestSubsequences(String x, String y, int[][] dp, int row, int col, String output, Set<String> set) {
        if (row <= 0 || col <= 0) {
            while(row>0){
        ...
        if (x.charAt(row - 1) == y.charAt(col - 1)) {
            printAllLongestSubsequences(x, y, dp, row-1, col-1, x.charAt(row - 1) + output, set);
        } else {
            if (dp[row - 1][col] > dp[row][col - 1]) {
                printAllLongestSubsequences(x, y, dp, row-1, col, x.charAt(row - 1) + output, set);
            } else if (dp[row - 1][col] < dp[row][col - 1]){
                printAllLongestSubsequences(x, y, dp, row, col-1, y.charAt(col - 1) + output, set);
            } else{
                printAllLongestSubsequences(x, y, dp, row-1, col, x.charAt(row - 1) + output, set);
                printAllLongestSubsequences(x, y, dp, row, col-1, y.charAt(col - 1) + output, set);
            }
