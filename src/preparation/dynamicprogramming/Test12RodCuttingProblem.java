...
public class Test12RodCuttingProblem
{
    ...
    /*
    here desired length is same as weight, length array is maintained to suffice for wt array,
    price is for value array and
    size is for n...it can be done similarly for dp so not including. only change is not to
    decrement size in cutRod(price,size
    , length, desiredLength[size-1]) because same rod can be taken more than once, leading to
    unbounded knapsack
     */
    private static int cutRod(int[] price, int size, int[] length, int desiredLength)
    {
        ...
        {
            return Math.max(
                price[size - 1] + cutRod(price, size, length, desiredLength - length[size - 1]),
    ...
    private static int cutRodAnotherWay(int[] price, int size, int lengthTillNow, int priceTillNow)
    {
        ...
        for (int j = 0; j < size && (j + 1 + lengthTillNow <= size); j++)
        {
            ...
            max = Math.max(max,
                cutRodAnotherWay(price, size, lengthTillNow + j + 1, priceTillNow + price[j]));
    ...
    private static int cutRodAnotherWay2(int[] price, int size, int lengthTillNow, int priceTillNow,
        int max)
    {
        if (lengthTillNow == size)
        {
        ...
        for (int j = 0; j < size; j++)
        {
            ...
            {
                max = Math.max(max,
                    cutRodAnotherWay2(price, size, lengthTillNow + j + 1, priceTillNow + price[j],
                        max));
            }
