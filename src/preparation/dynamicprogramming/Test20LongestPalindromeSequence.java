...
public class Test20LongestPalindromeSequence
{
    ...
    {
        /*
        one way is to convert it into lcs form, reverse the string and find lcs of both str and
        reverse of string and that would be
        the answer
         */
        ...
        /*
        another way is that if last and first characters are same, then take them otherwise max
        of (leave first and take second,
        ...
        /*
        to print palindromic subsequence...first create matrix with lcs approach and then print
        all subsequences
         */

        System.out.print(
            "length of lps is : " + findLongestPalindrome(seq.toCharArray(), seq.length(), 0,
                n - 1));
    }
    ...
    private static int lps(char[] toCharArray, int i, int j, int[][] dp)
    {
        ...
        {
            //2 beacuse if characters are same and we consider them then length would be char at
            // i and char at j, i.e. 2
    ...
    {
        int lcsLength = Test15LongestCommonSubSequence.lcsDP(toCharArray, toCharArray1,
            toCharArray.length, toCharArray1.length);
        return lcsLength;
    }
    ...
    private static int findLongestPalindrome(char[] str, int len, int s, int e)
    {
        ...
        {
            return Math.max(findLongestPalindrome(str, len, s + 1, e),
                findLongestPalindrome(str, len, s, e - 1));
        }
    }
}
