...
public class Test06SubsetSumProblem {
    ...
    private static boolean isSubsetSumWithOptimization(int[] set, int n, int sum, boolean[][] dp) {
        ...
        if(n==0)
            return false; //if set size is 0, sum with a positive integer can't be possible with any set. however if sum is 0 and
    ...
    private static boolean isSubsetSum(int[] set, int n, int sum, int sumTillNow, Set<Integer> s) {
        ...
        if(n==0) {
            return false;
        }//if set size is 0, sum with a positive integer can't be possible with any set. however if sum is 0 and
                         // n=0 then empty set is possible
