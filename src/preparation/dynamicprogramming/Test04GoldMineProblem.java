...
public class Test04GoldMineProblem
{
    ...
    public static void main(String[] args)
    {
        ...
        /*
        very easy way..start from last column and iterate each column for all rows. For each row,
         replace gold[row][column] with
        max of right, right up and right down of gold[row][column]. Finally find max of all the
        rows at 0th column
    ...
    private static int getMaxGoldByDP(int[][] gold, int m, int n)
    {
        for (int col = n - 1; col >= 0; col--)
        {
            for (int row = 0; row < m; row++)
            {
                ...
                int ru =
                    row - 1 >= 0 && row - 1 < m && col + 1 >= 0 && col + 1 < n ? gold[row - 1][col
                        + 1] : 0;
                int rd =
                    row + 1 >= 0 && row + 1 < m && col + 1 >= 0 && col + 1 < n ? gold[row + 1][col
                        + 1] : 0;
    ...
    private static int getMaxGoldWithQueue(int[][] gold, int m, int n, boolean[][] visited, int r,
        int c)
    {
        ...
        while (!q.isEmpty())
        {
            ...
            for (int k = 0; k < xPositions.length; k++)
            {
                if (isValid(xPositions[k], m) && isValid(yPositions[k], n))
                {
                    ...
                    q.add(new Point(xPositions[k], yPositions[k],
                        point.cost + gold[xPositions[k]][yPositions[k]]));
                    //}
