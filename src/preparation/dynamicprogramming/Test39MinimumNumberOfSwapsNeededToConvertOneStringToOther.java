...
/*
we wil use previous approach only. here we have two strings so we will keep first pointer at end
of first string
...
public class Test39MinimumNumberOfSwapsNeededToConvertOneStringToOther
{
    ...
    public static void main(String[] args)
    {
        ...
        //can be done more easily if i and j taken from start...same thing just we have to
        // increment j and fix i, char at i and j are not same till char at i is equal to char
        //at any j...now that j is found, do a while of (i<j) and swap j with j-1 element. note
        // that i does not change so outer while loop will work but for inner loop, we
    ...
    private static int CountSteps(char[] toCharArray1, char[] toCharArray2, int n)
    {
        ...
        while (initPointer >= 0)
        {
            ...
            {
                //now the characters are not same so find j such that value of char at j is equal
                // to value of char
                //at i
                int cnt = p1 - 1;
                ...
                //this will not happen as we have already checked that both strings are anagrams,
                // so can be removed
                if (cnt < 0)
                {
                    return -1;
