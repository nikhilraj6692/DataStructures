...
/*
this is an exact variation of Test06SubsetSumProblem. the only thing is that we do not have to shortcircuit the recursion or matrix
once we get true or false. instead we have to check for all occurrences and do a plus of with and without condition. Also, we have to
replace true by 1 and false by 0. 1 meaning that there can be one subset for combination of i amd j and 0 for no subset
 */
public class Test08CountSubsetsWithAGivenSum {
    ...
    private static int countSubsetsWithOptimization(int[] set, int n, int sum, int[][] dp) {
        if(sum==0 && n==0)
            return 1;
        if(n==0)
            return 0; //if set size is 0, sum with a positive integer can't be possible with any set. however if sum is 0 and
        ...
        if(set[n-1]<=sum){
            dp[n][sum] = countSubsetsWithOptimization(set, n-1, sum-set[n-1], dp) + countSubsetsWithOptimization(set, n-1, sum, dp);
            return dp[n][sum];
        }else{
