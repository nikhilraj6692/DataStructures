...
public class Test17PrintLongestCommonSubsequence
{
    ...
    public static void printAllLongestSubsequences(String x, String y, int[][] memotized, int row,
        int col, String output)
    {
        if (row <= 0 || col <= 0)
        {
        ...
        {
            printAllLongestSubsequences(x, y, memotized, row - 1, col - 1,
                output + x.charAt(row - 1));
        } else
        {
    ...
    public static void printLongestSubsequenceSum(String x, String y, int m, int n,
        int[][] memotized, Stack<Character> stack)
    {
        int i = m, j = n;
