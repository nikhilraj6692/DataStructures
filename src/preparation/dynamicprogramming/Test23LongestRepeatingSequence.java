...
/*
In this problem, we have to find longest repeating sub sequence such that the repeating sub
sequence order should be maintained
but it need not be continuous (sub sequence property). eg, for ABBA, the only repeating
subsequence would be either A or B,
for aabb, it can be ab. To solve this we will have to use lcs. We will take second string same as
 the first one and we will
...
public class Test23LongestRepeatingSequence
{
    ...
    private static String printSubsequence(char[] arr, int row, int column, int[][] dp,
        String output)
    {
