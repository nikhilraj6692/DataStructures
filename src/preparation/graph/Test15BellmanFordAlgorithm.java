...
/*
The algorithm can check for shortest path for negative weights, which Dijktra algor cannot. But
it does not work for negative
weight cycles. Negative weight cycle means that if in a graph sum of vertices of a cycle results
in negative integer, then
...
public class Test15BellmanFordAlgorithm
{
    ...
    public static void main(String[] args)
    {
        ...
        /*
        In this algorithm, the iteration runs for N-1 times. With each iteration, edge in edges
        list iterates and the distance of
        a vertex v is updated if distance[u] + cost(u,v) is less than distance[v]. By the end of
        all the iteration of N-1 count,
        ...
        /*
        parent array to maintain parent vertex of each node, so as to print path from any vertex
        to any other vertex
         */
    ...
    private static void doBellmanFordAlgorithm(int[] distance, int[] parent,
        List<Edge<Integer>> edges, int n)
    {
        for (int i = 0; i < n - 1; i++)
        {
        ...
        /*
        check if there is a negative weight cycle. If after all the iterations, the distance is
        updated, it means that shortest path
