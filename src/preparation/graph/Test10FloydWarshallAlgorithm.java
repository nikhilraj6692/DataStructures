...
public class Test10FloydWarshallAlgorithm
{
    ...
    public static void main(String[] args)
    {
        ...
        /*
        In this algorithm, we iterate a loop of k as each vertex and puts it as an intermediate
        vertex between i and j vertices
        of adjacency matrix. If by change, the sum of distance[i][k] and distance[k][j] sum comes
         to be smaller than direct
        distance between i and j, i.e distance[i][j], then update the distance matrix. To
        determine the path, we create a path matrix
         */
    ...
    private static void floydWarshall(int[][] adjMatrix, int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                //doing this because lets say that a shortest path exists between two vertex
                // 0 and 1 as 0->1, then when we go to
