...
public class Test25FindAMotherOrRootVertexOfAGraph
{
    ...
    /*
    Case 1:- Undirected Connected Graph : In this case, all the vertices are mother vertices as
    we can reach to all the other nodes
     in the graph.
    Case 2:- Undirected/Directed Disconnected Graph : In this case, there is no mother vertices
    as we cannot reach to all the other
    nodes in the graph.
    Case 3:- Directed Connected Graph : In this case, we have to find a vertex -v in the graph
    such that we can reach to all the
    other nodes in the graph through a directed path.

    One naive approach is to iterate every vertex and find dfs. at the end of each iteration, we
    check if all vertices are
    visited. If yes, then that is the mother vertex. otherwise check for next vertex and so on..
    .but one other way to do it is
    by making a note of last visited vertex if the vertex is not visited and finally do a dfs for
     the vertex. if it covers all
