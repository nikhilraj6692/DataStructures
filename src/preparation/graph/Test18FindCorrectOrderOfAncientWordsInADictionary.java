...
public class Test18FindCorrectOrderOfAncientWordsInADictionary
{
    ...
    public static void main(String[] args)
    {
        ...
        for (List<String> characterListInAWord : dict)
        {
            ...
            {
                //k++ will assign a new digit to each character which will act as a vertex in a
                // graph
        ...
        /*
        now create edges. since it is a dictionary, so we can compare two words in the list. IF
        the character mismatches then it is certain that
        character in previous word would be smaller than character in the next word, so we create
         a directed edge out of both
        characters and then we have to break because we can't comment about next character. If
        both character matches then we have
    ...
    private static void performTopologicalSort(Graph graph, boolean[] visited, Stack<Integer> stack,
        int src)
    {
        visited[src] = true;
