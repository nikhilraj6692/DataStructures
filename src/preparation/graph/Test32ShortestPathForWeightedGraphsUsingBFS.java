...
/*
As we know that BFS only works with graphs with same weight or no weight. For weighted graphs, there are different other algo. But
for BFS if the weights are in multiple of some integer x, like if a graph as 1,2,3 weight edges, then we can achieve bfs by splitting
edges into n edges where edge weight is equal to n*x and x is base weight. like 3 can be split into 3 edges with each of weight 1.
