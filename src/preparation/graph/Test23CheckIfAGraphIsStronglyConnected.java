...
public class Test23CheckIfAGraphIsStronglyConnected
{
    ...
    public static void main(String[] args)
    {
        ...
        /*
        to check if graph is strongly connected meaning that there is a path from every vertex is
         reachable from every other
        vertex. Below approach will iterate for all the vertices and check if visited array is
        having all vertices as visited or
        ...
        System.out.println(
            isConnected ? "Graph is strongly connected" : "Graph is not strongly connected");

        /*
        To improve time complexity, there is another algorithm, i.e. Kosaraju's DFS based
        algorithm. The algorithm says that
        if there is a path from any vertex v to other vertices then there would be definitely a
        path from any other vertices to
        v. So, we do two DFS traversals. One is with the edge from v to any other vertex and
        other reversed. If any one of the
        traversal has visited array not true from all elements, then graph is not strongly
        connected. Can also be done by BFS with
        queue approach
         */
    ...
    private static boolean checkIfGraphIsConnectedbyKosarajuAlgo(int src, Graph graph,
