...
public class Test09DijktraAlgorthm_ShortestPathAlgorithm {
    public static void main(String[] args) {
        ...
        /*
         In this algorithm, start from src and update its distance to 0 in a distance array. Initialize remaining element to max
         number.The distance array holds the distance of each and every vertex from source node. Check for adjacent vertices of the
         visiting vertex and update the adjacent vertex distance (minimum) in case the vertex is still not visited. To find out which
         vertex to chose from set of adjacent vertices, use minheap or min,minIndex variables. Do this until the queue is empty or
         min heap is empty in case it is solved by min heap. Print out the distance array to get the minimum cost of visiting each
         vertex from source vertex. To find out the parent of a vertex recursively from source, maintain a prevArray which will
    ...
    private static void performDijktraAlgorithmWithMinHeap(Graph graph, int n, int src) {
        ...
        /*
        priority queue automatically sorts the values on the basis of comparator provided in it. it will contain vertex and its
        distance and the vertices would be sorted on the basis of its distance
         */
        PriorityQueue<GNode> minHeap = new PriorityQueue<>(Comparator.comparingInt(node-> node.weight));
        ...
        for(int i=0; i<distanceArray.length; i++){
            System.out.println(i + "\t\t\t" + distanceArray[i]  + "\t\t\t\t\t " + findPathUtil(prevArray, i, src));
        }
