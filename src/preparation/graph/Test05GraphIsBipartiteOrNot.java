...
public class Test05GraphIsBipartiteOrNot
{
    ...
    public static void main(String[] args)
    {
        ...
        /*A graph is a bipartite graph if the node to be visited has adjacent node of a different
         colour. Here in this graph,
         while doing DFS traversal 2 is a visited node which was assigned a color, lets say false
          and 4 is also visited which
        was a assigned a colour as false. But while visiting edge (2,4) both are of same colour,
        so this is not a bipartite graph
        There is one more condition that the graph cannot contain an odd cycle. For odd cycles,
        check for level of destination and
    ...
    private static boolean is2Colorable(Graph graph, boolean[] visited, boolean[] colorable,
        int src)
    {
        visited[src] = true;
        boolean val = true;
