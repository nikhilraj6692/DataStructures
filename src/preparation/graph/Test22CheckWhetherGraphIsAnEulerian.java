...
/*
some key notes:
an undirected graph has an eurelian path if zero or two vertices have odd degree and all vertices
 with non-zero degree should belong to a
single connected component (meaning that after all vertices is done with traversal, if any of the
 vertex remains unvisited
then it is not single connected graph and there may more than 1 connected graphs). an undirected
graph has an eurelian circuit if
all vertices have odd degree and all vertices with non-zero degree should belong to a single
connected component. an undirected
graph has a eurelian path but not circuit, i.e. exactly two vertices have odd degree and all
vertices with non-zero degree should
belong to a single connected component.

An eurelian path is also called eurelian trail. It is a path that visits every edge in a graph
exactly only once. An eurelian circuit
is also called eurelian trail if it starts and ends on the same vertex. A directed graph has
eurelian cycle if and only if each vertex
