...
/*
can be done in a linear time for directed acyclic graph but may be a np-hard problem for directed
 graph which does not mention
...
public class Test19LongestPathInDirectedAcyclicGraph
{
    ...
    public static void main(String[] args)
    {
        ...
        System.out.print("\n------------------------------\n");
        /*
        There is yet another to do it in linear time. First sort the vertices in topological sort
         and then call bellman ford algo
    ...
    private static void doBellmanFordForLongestPathWithStack(Stack<Integer> stack, int[] distance,
        Graph graph, int[] parent)
    {
        while (!stack.isEmpty())
        {
            int v = stack.pop();
    ...
    private static void doBellmanFordForLongestPath(List<Edge<Integer>> edges, int[] distance,
        int n, int[] parent)
    {
        for (int i = 1; i < n; i++)
        {
            /*
            instead of iterating through the edges, we can also iterate adjlist.get(n), where n
            is 0 to adjlist.size()-1 inclusive
             */
            for (Edge edge : edges)
            {
                if (distance[edge.source] != Integer.MIN_VALUE
