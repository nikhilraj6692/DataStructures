...
public class Test27PathFromSourceToDestinationAirports {
    public static void main(String[] args) {
        /*
        departure is scheduled from evert airport except final destination and each airport is visted only once...no cycle
        ...
        /*
        check for source airport (it would be the one which is not present in destination airports) and then if it is formed as
    ...
    private static String findSourceAirport(Map<String, String> tickets) {
        ...
        Optional<String> srcOptional = tickets.keySet().stream().filter(src->!destinationAirports.contains(src)).findFirst();
