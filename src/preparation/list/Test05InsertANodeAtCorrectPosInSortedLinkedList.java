...
/*
to do with extra pointer, we can keep a reference of previous node so that we can always connect prevNode.next = new node
and new node.next = temp.next when temp.data is greater than new node data...but without extra pointer start from temp.next
...
public class Test05InsertANodeAtCorrectPosInSortedLinkedList {
    ...
    private static Node<Integer> sortedInsertWithoutExtraPointer(Node<Integer> head, Node<Integer> node) {
        if(head == null)
            return null;
    ...
    private static Node<Integer> sortedInsert(Node<Integer> head, Node<Integer> node) {
        ...
        //check if node to be inserted is less than the head element. point node to head and node as head
        if(node.data <= temp.data){
            node.next = temp;
            temp = node;
            head = temp;
        }else{
            //iterate till node data is greater than or equal to temp data. Maintain previous node and next node as temp.
