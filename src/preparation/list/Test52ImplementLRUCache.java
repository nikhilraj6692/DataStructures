...
/*
The algorithm can be implemented through a hashmap and doubly linked list where map will do get
operation in O(1) time and
linked list will be able to insert and remove element in O(1) time. In linked list older one at
the head and later
...
class LRUCache
{
    ...
    public void refer(Integer elem)
    {
        ...
        //update elem in the map , because it might happen that the node is already present in
        // map but its link has to be
        //changed again
        map.put(elem, node);
