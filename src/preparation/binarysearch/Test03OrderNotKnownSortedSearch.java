...
public class Test03OrderNotKnownSortedSearch {
    public static void main(String[] args) {
        ...
        /*
        here order is not known so just check if the order is ascending or descending, also check for edge case
         */
    ...
    private static int binarySearch(int[] arr, int start, int end, int element) {
        if (arr.length == 1) {
            ...
            if (order == -1)
                return Test02BinarySearchOnReverseSortedArray.binarySearch(arr, start, end, element);
            else
