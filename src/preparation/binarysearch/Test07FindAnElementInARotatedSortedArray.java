...
public class Test07FindAnElementInARotatedSortedArray {
    ...
    /*
    search in left if start is less than mid, then search for key if key is between start and mid then search in left or search
     in right
     */
    private static int pivotedBinarySearchRecursiveOnePass(int[] arr, int n, int key, int start, int end) {
        if(start > end)
            return -1;
    ...
    /*
    same question as before...we can find out the pivot(min element in array) before which array is rotated and from which array
     is not rotated.Now we got two sub arrays. Find element in any one of the two. If found, then return otherwise return -1;
     */
