...
/*
a min heap is a heap which is a complete binary tree and its parent is smaller than its left node
 and right node
 */
public class Test35CheckIfTreeIsHeapOrNot
{
    ...
    private static boolean checkIfATreeIsMinHeapRecursive(Node<Integer> node, Node<Integer> parent)
    {
        ...
        return checkIfATreeIsMinHeapRecursive(node.left, node) && checkIfATreeIsMinHeapRecursive(
            node.right, node);
    ...
    private static int checkIfATreeIsMinHeap(Node<Integer> root)
    {
        ...
        while (!q.isEmpty())
        {
            ...
            if ((temp.left != null && temp.right == null) || (temp.right != null
                && temp.left == null))
            {
                return -1;
            }
