...
public class Test09CousinNodes
{
    ...
    private static void findCousinsOrNot(Node<Integer> node, int firstNode, int secondNode)
    {
        ...
        map.entrySet().stream().forEach((kv) -> {
            System.out.println(
                kv.getKey().data + " , { " + (null != kv.getValue().first ? kv.getValue().first.data
                    : "") + " , " + kv.getValue().second + " } ");
            if (kv.getKey().data == firstNode)
            {
                nodeInfo1.set(
                    new NodeInfo(kv.getKey().data, kv.getValue().second, kv.getValue().first));
        ...
        if (nodeInfo1.get().parent != nodeInfo2.get().parent
            && nodeInfo1.get().level == nodeInfo2.get().level)
        {
