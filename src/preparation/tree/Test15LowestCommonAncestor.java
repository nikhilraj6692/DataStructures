...
public class Test15LowestCommonAncestor
{
    ...
    /*
    find the root from root to node1, keep it in an array. Similarly, for node2 and then match
    the elements in both arrays.
    if the element is not present in the path after iterating node's left and right then remove
    the last element from the array
     */
    private static int findLCAApproach1(Node<Integer> root, Node<Integer> node1,
        Node<Integer> node2)
    {
    ...
    private static boolean findPathApproach1(Node<Integer> root, Node<Integer> node,
        List<Integer> list)
    {
        if (root == null)
        {
