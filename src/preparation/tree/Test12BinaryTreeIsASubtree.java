...
/*
inorder and postorder or inorder and preorder is a length method...we can solve by checking for each node with
the subtree node. if data matches, it means that we have to go to check if left and right is also equal..If at
any point data is not equal, then check for left and subtree, similarly check for right and subtree. anyone should
be true.
 */
public class Test12BinaryTreeIsASubtree {
    ...
    private static boolean isSubtreeRecursive(Node<Integer> tree, Node<Integer> subTree) {
        //doing this because we will go on shortening tree and subtree. if a subtree lies in a tree then
    ...
    private static boolean isSubtree(Node<Integer> node1, Node<Integer> node2) {
        if (toString(inorder(node1)).contains(toString(inorder(node2))) && toString(preorder(node1)).contains(toString(preorder(node2)))) {
            return true;
        }
        return false;
